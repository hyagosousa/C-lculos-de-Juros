...
<div class="container dashboard">
    ...
    <p>Total Recebido: <span id="totalRecebido" class="valor-total">R$ 0,00</span></p>
...
function atualizarCalculo(){
    ...
    prevValor.innerText = formatarMoeda(valor);
    prevJuros.innerText = formatarMoeda(vJuros);
    prevFinal.innerText = formatarMoeda(vFinal);
}
...
function salvarCliente(){
    ...
    let valor = parseFloat(valorInput.value) || 0;
    let juros = parseFloat(jurosInput.value) || 0;
    let dataEmp = dataEmpInput.value;
    let dataVenc = dataVencInput.value;
    ...
    clientes.push({
        ...
        valor, juros, valorJuros, valorFinal,
        dataEmp, dataVenc,
        pago: false
    });
...
function cobrar(telefone, valorFinal, dataVenc, nome){
    let msg = `Olá ${nome}, sua dívida de ${formatarMoeda(valorFinal)} vence em ${dataVenc}. Favor realizar o pagamento.`;
    window.open(`https://wa.me/55${telefone}?text=${encodeURIComponent(msg)}`);
}

function enviarAlertaAtrasados(){
    let hoje = new Date();
    let atrasados = clientes.filter(c => !c.pago && new Date(c.dataVenc) < hoje);

    if(atrasados.length === 0){
        alert("Nenhum atrasado.");
        return;
    }
...
function calcularTotais(){
    ...
    clientes.forEach(c => {
        if(c.pago){
            pagos++;
            totalRecebido += c.valorFinal;
        } else {
            pendentes++;

            if(new Date(c.dataVenc) < hoje) atrasados++;
    ...
    totalEmpGeral.innerText = formatarMoeda(totalEmp);
    totalJurosGeral.innerText = formatarMoeda(totalJuros);
    totalReceberGeral.innerText = formatarMoeda(totalFinal);
    totalRecebido.innerText = formatarMoeda(totalRecebido);

    totalClientes.innerText = clientes.length;
    totalPagos.innerText = pagos;
    totalPendentes.innerText = pendentes;
    totalAtrasados.innerText = atrasados;
}
...
function atualizarTabela(){
    ...
    let busca = buscar.value.toLowerCase();
    let filtro = filtroStatus.value;
    let hoje = new Date();

    clientes.forEach((c, i) => {
        let atrasado = !c.pago && new Date(c.dataVenc) < hoje;

        if(busca && !(c.nome.toLowerCase().includes(busca) || c.cpf.includes(busca))) return;
        if(filtro === "pago" && !c.pago) return;
        if(filtro === "pendente" && (c.pago || atrasado)) return;
        if(filtro === "atrasado" && !atrasado) return;

        let tr = document.createElement("tr");
        tr.style.background = c.pago ? "#c8f7c5" : atrasado ? "#ffb3b3" : "white";

        if(atrasado) tr.classList.add("piscar");
...
function ordenarTabela(campo){
    clientes.sort((a,b) => a[campo] > b[campo] ? 1 : -1);
    atualizarTabela();
}
</script>
